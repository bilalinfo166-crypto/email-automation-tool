...
adapter = requests.adapters.HTTPAdapter(pool_connections=60, pool_maxsize=60, max_retries=1)
...
def _get(url, to=None):
    ...
    try:
        ...
        # Accept 200-399 always. Accept 403 if it has HTML content (Cloudflare pages)
        if r.status_code >= 500: return None
        if r.status_code >= 400 and r.status_code != 403: return None
        ...
        # SSL failed — try without verify (already False, try HTTP)
        try:
            r=_s.get(url.replace("https://","http://"), timeout=to or PAGE_TIMEOUT, allow_redirects=True)
...
def extract_domain(domain, mode="vendor"):
    ...
    # 1) HOMEPAGE — try multiple variations
    base="https://"+root
